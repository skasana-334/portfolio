      ...
      <div className='max-w-screen-xl mx-auto px-4 py-8 text-center md:text-left'>
        ...
        <div className='max-w-5xl grid md:grid-cols-2 gap-20'>
          {portfolios.map(({id,title,src,url}) => (
            <Link key={id} href={`${url}`}>
              <div className='cursor-pointer group shadow-md shadow-sky-600 overflow-hidden rounded-md'>
                ...
                />
                <h2 className='text-center text-base capitalize group-hover:underline underline-offset-4 duration-200 my-2 text-violet-500'>
                  {title}
                </h2>
              </div>
            </Link>
          ...
          <div className='flex justify-center items-center'>
            <Link href='/portfolio'>
              <div className='bg-green-500 p-2 font-semibold uppercase flex justify-center items-center text-white rounded-md px-3 my-6 tracking-wider group cursor-pointer'>
                All Projects
                <span className='-rotate-90 group-hover:translate-x-3'>
